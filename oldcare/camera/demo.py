...
def run_demo(net, action_net, image_provider, height_size, cpu, save_path):
    net = net.eval()
    if not cpu:
        net = net.cuda()
    ...
    for j in image_provider:
        height, width, _ = j.shape
        break
    print(0)
    fourcc = cv2.VideoWriter_fourcc('D', 'I', 'V', 'X')  
    print(1)  
    writer = cv2.VideoWriter(save_path, fourcc, 20, (width, height))
    print(2)
...
def detect_main(video_source='', image_source='', video_name='', save_path=''):
    ...
    net = jit.load('../../models/openpose1.jit')
    ...
    run_demo(net, action_net, frame_provider, args.height_size, args.cpu, save_path)


if __name__ == '__main__':
    detect_main(video_source='data/down1.mp4', save_path = 'video_result.mp4')
